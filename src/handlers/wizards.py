...
def _format_username_markdown(username: str | None) -> str:
    return escape_markdown(f"@{username}", version=1) if username else "Unknown"
...
async def newkey_ask_limit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        queries.record_key_data_grant(
            alias,
            str(new_key.key_id),
            limit_bytes if limit_gb > 0 else 0,
            customer_user_id=None,
            is_renewal=False,
            is_unlimited=limit_gb <= 0,
            metadata={
                "source": "new_key_creation",
        ...
        key_name_safe = _format_text_markdown(key_name, default="Unnamed")
        if limit_gb > 0:
