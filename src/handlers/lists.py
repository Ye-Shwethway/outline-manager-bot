...
def _reconcile_live_quota_block(alias: str, key, lifecycle: dict) -> dict:
    ...
    if queries.clear_key_quota_block_if_restored(alias, key_id, lifecycle, key.data_limit, raw_used_bytes):
        return queries.get_key_lifecycle(alias, key_id) or {}
    return lifecycle
...
def _build_renew_key_snapshot(alias: str, key_id: str) -> tuple[dict | None, str | None]:
    ...
    lifecycle = _reconcile_live_quota_block(alias, target_key, lifecycle)
...
async def handle_listkeys_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for key in keys:
            ...
            lifecycle = _reconcile_live_quota_block(alias, key, lifecycle)
