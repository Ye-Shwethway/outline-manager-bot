...
def _status_tag(key, sold_keys: set[str], effective_used_bytes: int) -> str:
    used_up = bool(key.data_limit) and effective_used_bytes >= key.data_limit
    if used_up:
        return "USED_UP"
...
def _usage_lines(key, effective_used_bytes: int) -> tuple[str, str]:
    used_gb = effective_used_bytes / BYTES_PER_GB
    if key.data_limit:
        limit_gb = key.data_limit / BYTES_PER_GB
        available_gb = max(key.data_limit - effective_used_bytes, 0) / BYTES_PER_GB
        return (
    ...
    if not servers:
        ...
        for alias in sorted(servers.keys()):
            ...
            for key in keys:
                ...
                usage, available = _usage_lines(key, effective_used_bytes)
